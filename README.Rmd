...
To add a custom design into the VPalm app, you should follow the format of `design_plot()`. For example for 2 rows of 2 palm trees in quincunx (*i.e.* 4 palms), we 
call `design_plot()` as follows:
...
library(knitr)
library(Vpalmr)
design= design_plot(rows = 2, cols = 2, x_dist = 9.2)
kable(design$design)
```
